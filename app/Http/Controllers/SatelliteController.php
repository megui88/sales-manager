...
class SatelliteController extends Controller
{
    ...
    public function downloadFile($period)
    {
        $rows = DB::select( DB::raw(
            "SELECT u.code, " .
            " u.headquarters_id," .
            " SUM(d.amount_of_quota) as mount FROM users as u ".
            "JOIN dues as d ON d.period = :period AND d.payer_id=u.id " .
    ...
    public function retiradoFile($period)
    {
        $rows = DB::select( DB::raw(
            "SELECT u.code, " .
            " u.headquarters_id," .
            " SUM(d.amount_of_quota) as mount FROM users as u ".
            "JOIN dues as d ON d.period = :period AND d.payer_id=u.id " .
            "WHERE u.role != 'proveedor' AND u.state='" . BusinessCore::MEMBER_AFFILIATE . "'AND u.company_id='b3cddbce-9092-11e6-9521-04011111c601' " .
            "GROUP BY d.payer_id ORDER by u.headquarters_id, u.code"),
            [ 'period' => $period,]
        );
        ...
        foreach ($rows as $row){
            $code = ($row->headquarters_id =='b3d519f2-9092-11e6-9ece-04011111c601') ? 5132 : 5124;
            $content .= $row->code . ';' . number_format($row->mount, 2 , ',', '') . ';'. $code. PHP_EOL;
        }
        return Response::make($content)
            ->header("Content-type"," charset=utf-8")
            ->header("Content-disposition","attachment; filename=\"RETIRADOS_".$period."_MUTUALMP.csv\"");
    }
}
